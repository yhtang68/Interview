...
| decimal.js | `10.6.0` | Decimal arithmetic for financial calculations | [decimal.js - npm README](https://www.npmjs.com/package/decimal.js) |
...
| [`src/services/crd_PortfolioService.ts`](./src/services/crd_PortfolioService.ts) | - Defines the readable CRD portfolio endpoint tree.<br>- Calls the mocked product endpoint.<br>- Lists, clears, and resets mocked portfolio accounts.<br>- Refreshes the mocked account-name collection when dynamic accounts are published.<br>- Creates and updates scenario-owned WireMock portfolio fixtures. |
| [`src/services/crd_PortfolioModel.ts`](./src/services/crd_PortfolioModel.ts) | - Defines the CRD portfolio data contract.<br>- Validates portfolio and security response payloads.<br>- Uses decimal arithmetic for financial calculations.<br>- Preserves authoritative `Total Asset` metadata and validates security current values against allocation percentages.<br>- Refreshes derived asset metadata.<br>- Models whole-share trades and preserves remaining value as `CRD_CASH`. |
...
| [`wiremock/__files/portfolioService/README.md`](./wiremock/__files/portfolioService/README.md) | Documents the static portfolio-service fixture layout, the current `accounts[]` uniqueness assumption, and the dynamic portfolio identity simplification. |
